...
@section('content')
    <section class="max-w-6xl" style="margin-top: 4px;">
        ...
        @if ($courses->isEmpty())
            <div class="text-center py-16">
                ...
                <a href="{{ route('courses.index') }}"
                    class="inline-flex items-center px-8 py-3 bg-accent text-white rounded-btn hover:bg-accent-hover transition font-semibold">
                    Browse Courses
                </a>
            </div>
        @else
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($courses as $course)
                    <div
                        class="bg-white rounded-card shadow-card hover:shadow-card-hover transition-shadow overflow-hidden group">
                        <a href="{{ route('courses.show', $course->slug) }}">
                            <div class="aspect-video bg-gray-100 overflow-hidden">
                                ...
                                @else
                                    <div class="w-full h-full flex items-center justify-center text-muted text-sm">No
                                        Thumbnail</div>
                                @endif
                            </div>
                        </a>
                        <div class="p-5">
                            @if ($course->category)
                                <span
                                    class="text-xs text-accent font-medium uppercase tracking-wide">{{ $course->category->name }}</span>
                            @endif
                            <a href="{{ route('courses.show', $course->slug) }}">
                                <h3 class="font-semibold text-base mt-1 line-clamp-2 hover:text-accent transition">
                                    {{ $course->title }}</h3>
                            </a>
                            ...
                            @else
                                <div
                                    class="mt-4 block w-full text-center px-4 py-2 bg-gray-100 text-gray-400 text-sm rounded-btn font-medium">
                                    Content Coming Soon
                                </div>
                            @endif
                        </div>
